### Funkcia na spocitanie celkoveho vynosu z jedneho produktu s DPH za vybrane obdobie
...
### Rovnaka funkcia ale s moznostou vyberu s DPH / bez DPH
  ...
  IF include_vat_param THEN
    SELECT NVL(SUM((p.price + p.price * v.rate / 100) * uo.amount), 0)
     ...
     WHERE uo.products_id = product_id_param
       AND o.created_at BETWEEN start_date_param AND end_date_param;
  ELSE
    SELECT NVL(SUM(p.price * uo.amount), 0)
      INTO total_profit
      FROM user_orders uo
           JOIN orders o ON uo.orders_id = o.id
           JOIN prices p ON uo.prices_id = p.id
...
SELECT calculate_product_profit(1, TO_DATE('2025-03-18', 'YYYY-MM-DD'), TO_DATE('2025-03-20', 'YYYY-MM-DD'), FALSE) AS total_profit;
```
